...
class SimulatedTrader:
    ...
    def stop(self):
        ...
        logger.info(f"[{self.name}] Stopped. Stats: {self.orders_sent} orders sent, {self.responses_received} responses received")
...
class SimulatedTradersManager:
    ...
    def generate_stats_table(self):
        ...
        for s in stats:
            ...
            response_rate = f"{(s['responses_received'] / s['orders_sent'] * 100):.1f}%" if s['orders_sent'] > 0 else "0.0%"
        ...
        if stats:
            table.add_section()
            overall_rate = f"{(total_responses / total_orders * 100):.1f}%" if total_orders > 0 else "0.0%"
            table.add_row(
...
def run_simulated_traders(count: int = 5, trade_frequency: float = 5.0):
    ...
    try:
        ...
        console.print(f"[bold green]✓[/bold green] All traders started and placing orders...\n")
