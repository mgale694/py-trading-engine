...
def server(
    ...
    env: str = typer.Option(ENV, help="Environment: dev or prod"),
):
    """
    ...
    """
    Config(env=env)  # Load config for environment
...
@app.command()
def client(
    name: str = typer.Argument("trader", help="Client to start: [bold yellow]trader[/bold yellow]"),
    env: str = typer.Option(ENV, help="Environment: dev or prod"),
):
    """
...
def frontend(
    name: str = typer.Argument(
        ...,
        help="Frontend to start: [bold magenta]trader[/bold magenta], [bold magenta]analytics[/bold magenta], or [bold magenta]db-viewer[/bold magenta]",
    ),
    env: str = typer.Option(ENV, help="Environment: dev or prod"),
):
    """
    ...
    [bold magenta]db-viewer[/bold magenta]: Database Viewer - Browse schemas and data
    """
    Config(env=env)  # Load config for environment
    ...
    if name == "trader":
        ...
        console.print(
            Panel(
                "[bold magenta]Database Viewer[/bold magenta]\n"
                "Browse database schemas, tables, and run SQL queries\n"
                "URL: [link]http://localhost:8501[/link]",
                title="🗄️ Starting Database Viewer",
                border_style="cyan",
            )
        )
        logger.info("Starting Database Viewer")
        subprocess.run(["streamlit", "run", "src/frontend/db-viewer/app.py"])
...
@app.command()
def init_mock_data(env: str = typer.Option(ENV, help="Environment (must be dev)")):
    """
    ...
    if env != "dev":
        console.print(
            "[bold red]Error:[/bold red] Mock data initialization is only available in [bold cyan]dev[/bold cyan] environment"
        )
        raise typer.Exit(1)
...
def simulated_traders(
    count: int = typer.Argument(..., help="Number of simulated traders to start"),
    env: str = typer.Option(ENV, help="Environment (must be dev)"),
):
    """
    ...
    if env != "dev":
        console.print(
            "[bold red]Error:[/bold red] Simulated traders are only available in [bold cyan]dev[/bold cyan] environment"
        )
        raise typer.Exit(1)
